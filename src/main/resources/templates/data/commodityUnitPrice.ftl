          ...
          <div class="right_col" role="main">
              <div class="">
                  ...
                  <div class="row">
                      ...
                      <div id="commodityUnitPrice" style="width: 100%;height:700px"></div>
                  </div>
              </div>
          </div>
      </div>
...
<script>
    ...
    var myChart = echarts.init(document.getElementById('commodityUnitPrice'));
    myChart.setOption({
        tooltip: {
            trigger: 'axis'
        },
    ...
    $.get('${ctx!}/data/getCommodityUnitPrice?strDate=' + strDate + '&endDate=' + endDate).done(function (data) {
        // 填入数据
        myChart.setOption({
            tooltip: {
                trigger: 'axis'
    ...
    function select() {
        ...
        $.get('${ctx!}/data/getCommodityUnitPrice?strDate=' + strDate1 + '&endDate=' + endDate1).done(function (data) {
            // 填入数据
            myChart.setOption({
                tooltip: {
                    trigger: 'axis'
    ...
    function sameDay(){
        ...
        var sameDate = $("#sameDate").val();
          $.get('${ctx!}/data/getCommodityUnitPrice?strDate=' + sameDate + '&endDate=' + sameDate).done(function (data) {
              // 填入数据
              myChart.setOption({
                  tooltip: {
                      trigger: 'axis'
