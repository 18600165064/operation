                              ...
                              <form id="form" class="form-horizontal" method="post" enctype="multipart/form-data">
						            <table class="table table-bordered">
						              ...
						              <tr>
						                ...
						                <td width="30%"  width="30%"  colspan="11">
						                	  <input type="hidden" name="sas_id" id="sas_id" />
						              ...
						              <tr>
						                ...
						                <td width="30%"  width="30%"  colspan="11">
							                  <select class="form-control"  id="status" name="status"  onchange="fun1()">
...
function tableFun2(tdUrl){
	$.ajax({
		...
		success : function(menuJson) {
			region= menuJson;
			var srhData = {"id" : _id};
			tableFun("${ctx!}/supplierAccountShipping/selectById",srhData);
		}
	})
...
function tableFun(tdUrl,tbData){
	 $.get(tdUrl,tbData, function(result){ 
			var menuJson = JSON.parse(result);
				  $("#nodel").val(menuJson.nodel);
				  $("#no_delivery").attr("data-id",menuJson.nodel);
				  $("#no_delivery").text(menuJson.nodelName);
				  $("#sas_id").val(menuJson.sas_id);
				  $("#templet_name").val(menuJson.templet_name);
				  $("#status").val(menuJson.status);
				  $("#default_price").val(menuJson.default_price);
				  $("#default_number").val(menuJson.default_number);
				  $("#increa_number").val(menuJson.increa_number);
				  $("#increa_price").val(menuJson.increa_price);
				  $("#type").val(menuJson.type);
				  
				  for (var i = 0; i < menuJson.supplierAccountShippingSettings.length; i++) {
				　　　 createRow(menuJson.supplierAccountShippingSettings[i]);
						$("#city"+i+"").attr("data-id",menuJson.supplierAccountShippingSettings[i].city);
						$("#city"+i+"").text(menuJson.supplierAccountShippingSettings[i].cityName);
						$("#id"+i+"").val(menuJson.supplierAccountShippingSettings[i].id);
						$("#shipping_id"+i+"").val(menuJson.supplierAccountShippingSettings[i].shipping_id);
						$("#first_tag"+i+"").val(menuJson.supplierAccountShippingSettings[i].first_tag);
						$("#first_price"+i+"").val(menuJson.supplierAccountShippingSettings[i].first_price);
						$("#second_tag"+i+"").val(menuJson.supplierAccountShippingSettings[i].second_tag);
						$("#second_price"+i+"").val(menuJson.supplierAccountShippingSettings[i].second_price);
				  }
...
function fun1(){
	$("#default_price").val("0.00");
	$("#default_number").val(0);
	$("#increa_number").val(0);
	$("#increa_price").val("0.00");
...
function createRow(data){
	...
	td3.width="8%";
	td3.innerHTML='<input type="text"  class="form-control validate[required,custom[onlyNumberSp]]" id="first_tag'+_int+'" name="supplierAccountShippingSettings['+_int+'].first_tag" placeholder="首件数" data-errormessage-value-missing="首件数不能为空"/>';
	var td4=document.createElement("td");
	td4.width="8%";
	td4.innerHTML='<em style="color:red;"> * </em>首费';
	var td5=document.createElement("td");
	td5.width="8%";
	td5.innerHTML='<input type="text"  class="form-control validate[required,custom[onlyNumberTwo]]" id="first_price'+_int+'" name="supplierAccountShippingSettings['+_int+'].first_price" placeholder="首费" data-errormessage-value-missing="首费不能为空"/>';
	var td6=document.createElement("td");
	td6.width="8%";
	td6.innerHTML='<em style="color:red;"> * </em>续件数';
	var td7=document.createElement("td");
	td7.width="8%";
	td7.innerHTML='<input type="text"  class="form-control validate[required,custom[onlyNumberSp]]" id="second_tag'+_int+'" name="supplierAccountShippingSettings['+_int+'].second_tag" placeholder="续件数" data-errormessage-value-missing="续件数不能为空"/>';
	var td8=document.createElement("td");
	td8.width="8%";
	td8.innerHTML='<em style="color:red;"> * </em>续费';
	var td9=document.createElement("td");
	td9.width="8%";
	td9.innerHTML='<input type="text"  class="form-control validate[required,custom[onlyNumberTwo]]" id="second_price'+_int+'" name="supplierAccountShippingSettings['+_int+'].second_price" placeholder="续费" data-errormessage-value-missing="续费不能为空"/>';
	...
	td11.width="1%";
	td11.innerHTML='<input type="hidden" class="form-control" name="supplierAccountShippingSettings['+_int+'].city" id="nodels'+_int+'"/> <input type="hidden" class="form-control" name="supplierAccountShippingSettings['+_int+'].id" id="id'+_int+'"/> <input type="hidden" name="supplierAccountShippingSettings['+_int+'].shipping_id" id="shipping_id'+_int+'" />';
...
function delRow(num){
	var _removeId = $("#id"+num+"").val();
	var tbody = document.getElementById("tbody");
	var cell = document.getElementById("tr"+num+"");
	tbody.removeChild(cell);
	if(_removeId != null && _removeId != ''){
		$.ajax({
			url:"${ctx!}/supplierAccountShippingSettings/delSupplierAccountShippingSettings",
	 		data : {"id":_removeId},
			dataType : "json",
			async : false,
...
function save(){
    ...
    var noDel = $("#no_delivery").attr("data-id");
	 ...
	 if(noDel != '' && noDel != null && noDel != 'undefind'){
		...
		$.ajax({
			url:"${ctx!}/supplierAccountShipping/editSupplierAccountShippingData",
	 		data : $("#form").serialize(),
			dataType : "json",
			async : false,
			success : function(data) {
					if (data.draw == 1) {
						bootbox.alert(data.error, function() {
							window.location.href = "${ctx!}/supplierAccountShipping/supplierAccountShippingMenu?s_id="+_sid+"";
						});
					}else{
