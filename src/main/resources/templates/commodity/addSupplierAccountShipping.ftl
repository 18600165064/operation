                              ...
                              <form id="form" class="form-horizontal" method="post" enctype="multipart/form-data">
						            <table class="table table-bordered">
						              ...
						              <tr>
						                ...
						                <td width="30%" width="30%"  colspan="11" >
						                  <input type="text"  class="form-control validate[required,custom[onlyNumberTwo],min[0]]" id="default_price" name="default_price" placeholder="默认运费" data-errormessage-value-missing="默认运费不能为空"/>
						                </td>
						              </tr>
						              <tr>
...
function createRow(){
	...
	td11.width="1%";
	td11.innerHTML='<input type="hidden" class="form-control" name="supplierAccountShippingSettings['+_int+'].city" id="nodels'+_int+'"/>';
...
function save(){
    ...
    var noDel = $("#no_delivery").attr("data-id");
	 ...
	 if(noDel != '' && noDel != null && noDel != 'undefind'){
		...
		$.ajax({
			url:"${ctx!}/supplierAccountShipping/addSupplierAccountShippingDate",
	 		data : $("#form").serialize(),
			dataType : "json",
			async : false,
			success : function(data) {
					if (data.draw == 1) {
						bootbox.alert(data.error, function() {
							window.location.href = "${ctx!}/supplierAccountShipping/supplierAccountShippingMenu?s_id="+_id+"";
						});
					}else{
