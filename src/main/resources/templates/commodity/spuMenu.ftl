...
var table = $('#datatable').DataTable({
	    ...
	    "columns": [
	        ...
	        },
	        {
	        	...
	        	"render":function(data, type, row) {
	        			if(row.on_sale == 1){
                            if(row.maintain_status == 2){
                                return '<a class="btn btn-danger btn-xs ml-5 js-delete" data-id="\'+data+\'" disabled="disabled" \\">维护中</a> <a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" href=\"${ctx!}/spuCommodity/editSpu?id='+data+'\">编辑</a> <a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" href=\"${ctx!}/topLevel/topLevelMneu?spuid='+data+'\">顶级账户</a>';
                            }else{
                                return '<a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" href=\"${ctx!}/spuCommodity/editSpu?id='+data+'\">编辑</a> <a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" href=\"${ctx!}/topLevel/topLevelMneu?spuid='+data+'\">顶级账户</a>';
                            }
	        			}else{
	        				if(row.maintain_status == 0){
	                        	return '<a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" onclick="seecontent('+data+')">查看</a> <a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" href=\"${ctx!}/spuCommodity/editSpu2?id='+data+'\">编辑</a> <a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" onclick="maintainSpu('+data+')"\">维护</a> <a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" href=\"${ctx!}/topLevel/topLevelMneu?spuid='+data+'\">顶级账户</a>';
		        			}if(row.maintain_status == 1){
	                        	return '<a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" onclick="seecontent('+data+')">查看</a> <a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" href="${ctx!}/spuCommodity/editSpu2?id='+data+'">编辑</a> <a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" disabled="disabled" \">开始维护</a> <a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" href=\"${ctx!}/topLevel/topLevelMneu?spuid='+data+'\">顶级账户</a>';
		        			}if(row.maintain_status == 2){
	                        	return '<a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" disabled="disabled" \">维护中</a> <a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" href=\"${ctx!}/spuCommodity/editSpu?id='+data+'\">编辑</a> <a class="btn btn-danger btn-xs ml-5 js-delete" data-id="'+data+'" href=\"${ctx!}/topLevel/topLevelMneu?spuid='+data+'\">顶级账户</a>';
		        			}
	        			}
...
//  	}
    ...
    function seecontent(id) {
        window.location.href = "${ctx!}/spuCommodity/seecontent?id="+id+"";
    }
