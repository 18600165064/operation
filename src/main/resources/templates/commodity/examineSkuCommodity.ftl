...
function tableFun(tdUrl,tbData){
	 $.get(tdUrl,tbData, function(result){ 
			var menuJson = JSON.parse(result);
				  ...
				  $("#refund_price").val(menuJson.refund_price);
...
function save(){
	if (!_errFm.validationEngine('validate')) {
		...
		var _id = $("#id").val();
		var _onSale = $("#on_sale").val();
		var _spuid = $("#spuid").val();
		var _srhData = {"id" : _id,"on_sale" : _onSale,"spuid":_spuid};
		$.ajax({
			url:"${ctx!}/product/editOnSale",
	 		data : _srhData,
			dataType : "json",
			async : false,
			success : function(data) {
					if (data.draw == 1) {
						bootbox.alert(data.error, function() {
							window.location.href = "${ctx!}/product/skuMenu2";
						});
					}else{
